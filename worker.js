...
async function ensureColumn(db,table,column,definition){
  ...
  if(!(info.results||[]).some(x=>x.name===column)) await db.prepare(`ALTER TABLE ${table} ADD COLUMN ${column} ${definition}`).run();
}
  ...
  try{
    ...
    if(method==='GET'&&path==='/admin'){
      const [st,sv,ap,cl,bl,n,summ]=await Promise.all([
        ...
        db.prepare("SELECT * FROM bf_daily_summaries ORDER BY date DESC").bind(today()).all()
      ]);
