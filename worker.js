...
async function onRequest(context){
  ...
  if(path==='/debug-public' && method==='GET'){
    if(!db) return json(503,{ok:false,step:'binding'});
    const out={ok:false,steps:[]};
    try { out.steps.push(['settings', await db.prepare('SELECT * FROM bf_settings WHERE id=1').first()]); }
    catch(e){ out.error='settings'; out.detail=String(e?.message||e); return json(500,out); }
    try { const r=await db.prepare('SELECT id,name,price,duration FROM bf_services WHERE online=1 ORDER BY name').all(); out.steps.push(['services_count',r.results?.length||0]); }
    catch(e){ out.error='services'; out.detail=String(e?.message||e); return json(500,out); }
    out.ok=true; return json(200,out);
  }
