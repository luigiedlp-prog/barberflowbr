...
async function dbInit(db){
  // Idempotent bootstrap: keep startup tolerant of an already-installed D1 schema.
  const statements = [
  ...
  for(const sql of statements){
    ...
    catch(e){ console.error('DB_BOOTSTRAP_STEP',sql.slice(0,80),e); }
  }
  let st=await db.prepare('SELECT * FROM bf_settings WHERE id=1').first();
  if(!st){
  ...
  if(Number(n?.n||0)===0){
    for(const s of SERVICES){
      await db.prepare('INSERT INTO bf_services(id,name,price,duration,online) VALUES(?,?,?,?,1)').bind(...s).run();
    }
  }
}
...
async function onRequest(context){
  ...
  try{
    ...
    if(method==='GET'&&path==='/public') { try { return json(200,await publicData(db)); } catch(e) { console.error('PUBLIC_DATA_ERROR',e); return json(500,{error:'Error interno',diagnostic:'PUBLIC_DATA_ERROR'}); } }
