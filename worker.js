...
async function rollover(db){
  ...
  for(const a of old.results){
    stm.push(db.prepare(`UPDATE bf_appointments SET status='no_show',no_show_at=CURRENT_TIMESTAMP WHERE id=?`).bind(a.id));
    stm.push(db.prepare('DELETE FROM bf_slots WHERE owner_id=?').bind(a.id));
    stm.push(db.prepare('UPDATE bf_clients SET no_shows=no_shows+1 WHERE id=?').bind(a.client_id));
  }
  await db.batch(stm);
}
...
async function canReserve(db,date,time,duration,excludeOwner=null){
  ...
  for(const t of slots){
    const x=excludeOwner ? await db.prepare('SELECT 1 FROM bf_slots WHERE date=? AND time=? AND owner_id<>?').bind(date,t,excludeOwner).first() : await db.prepare('SELECT 1 FROM bf_slots WHERE date=? AND time=?').bind(date,t).first();
    if(x) return false;
  }
  return true;
}
...
async function cancelAppointment(db,a,{late=false,source='customer',reason=''}={}){
  ...
  await db.batch([
    db.prepare(`UPDATE bf_appointments SET status='cancelled',cancelled_at=CURRENT_TIMESTAMP,late_cancel=?,cancel_source=?,cancel_reason=? WHERE id=?`).bind(late?1:0,source,reason||'',a.id),
...
if(!db) return json(500,{error:'Falta configurar el binding D1 llamado DB.'});
  try{
    ...
    if(method==='GET'&&path==='/recover-info'){const st=await settings(db);return json(200,{question:st.recovery_q});}
    ...
    if(method==='GET'&&path==='/admin'){
      ...
      let summaries=[];
      try{summaries=(await db.prepare("SELECT * FROM bf_daily_summaries ORDER BY date DESC").all()).results||[]}catch(e){console.error('SUMMARY_READ_ERROR',e);}
      return json(200,{settings:{address:st.address,phone:st.phone,recoveryQ:st.recovery_q},services:sv.results,appointments:ap.results,clients:cl.results,blocks:bl.results,notifications:n.results,summaries,schedule:DEFAULTS.schedule,today:today()});
    }
    ...
    if(method==='POST'&&path==='/admin/service'){
      ...
      if(body.id) await db.prepare('UPDATE bf_services SET name=?,price=?,duration=?,online=? WHERE id=?').bind(name,price,duration,body.online?1:0,body.id).run();
    ...
    if(method==='POST'&&path==='/admin/appointment'){
      ...
      if(!/^\d{4}-\d{2}-\d{2}$/.test(String(date||''))||!/^\d{2}:\d{2}$/.test(String(time||'')))return json(400,{error:'Fecha u hora inválida'});
      ...
      if(id){
        ...
        if(!validDate(date)||!await canReserve(db,date,time,sv.duration,id))return json(409,{error:'Horario ocupado o fecha inválida'});
      ...
      if(!clientId||!validDate(date)||!await canReserve(db,date,time,sv.duration))return json(409,{error:'Horario ocupado o datos inválidos'});
      await insertAppointment(db,{id:uid('a'),clientId,s:sv,date,time,status:'pending',source:'manual',notes}); return json(200,{ok:true});
    }
    if(method==='POST'&&path==='/admin/cancel'){
      ...
      await cancelAppointment(db,a,{source:'admin',reason:String(body.reason||'Cancelación realizada por Santi').trim()}); return json(200,{ok:true});
    }
    if(method==='POST'&&path==='/admin/no-show'){
      const a=await db.prepare('SELECT * FROM bf_appointments WHERE id=?').bind(body.id).first(); if(!a||a.status!=='pending')return json(400,{error:'Solo se puede marcar como no asistió un turno pendiente'});
      await db.batch([db.prepare(`UPDATE bf_appointments SET status='no_show',no_show_at=CURRENT_TIMESTAMP WHERE id=?`).bind(a.id),db.prepare('DELETE FROM bf_slots WHERE owner_id=?').bind(a.id),db.prepare('UPDATE bf_clients SET no_shows=no_shows+1 WHERE id=?').bind(a.client_id)]); return json(200,{ok:true});
    }
    if(method==='POST'&&path==='/admin/block'){
      ...
      if(affected.length&&body.cancelAffected){for(const a of affected){stm.push(db.prepare(`UPDATE bf_appointments SET status='cancelled',cancelled_at=CURRENT_TIMESTAMP,cancel_source='block',cancel_reason=? WHERE id=?`).bind(reason||'Bloqueo de horario',a.id));stm.push(db.prepare('DELETE FROM bf_slots WHERE owner_id=?').bind(a.id));stm.push(db.prepare('UPDATE bf_clients SET cancellations=cancellations+1 WHERE id=?').bind(a.client_id));stm.push(db.prepare(`INSERT INTO bf_daily_summaries(date,turns,revenue,cancelled) VALUES(?,0,0,1) ON CONFLICT(date) DO UPDATE SET cancelled=cancelled+1`).bind(a.date));stm.push(db.prepare('INSERT INTO bf_client_notifications(id,client_id,kind,title,body) VALUES(?,?,?,?,?)').bind(uid('cn'),a.client_id,'block_cancel','Turno cancelado',`Tu turno del ${a.date} a las ${String(a.time).slice(0,5)} fue cancelado porque el horario fue bloqueado.`));}}
